#include "GlucoseParaProver.h"

shared_ptr<Glucose::ParallelSolver> GlucoseParaProver::completeSolver = shared_ptr<Glucose::ParallelSolver>(new Glucose::ParallelSolver(0));


GlucoseParaProver::GlucoseParaProver(bool onesat) {
...
GlucoseParaProver::~GlucoseParaProver() {}

modal_names_map GlucoseParaProver::prepareSAT(FormulaTriple clauses,
                                          name_set extra) {
...
void GlucoseParaProver::prepareLtlfSat(LtlFormulaTriple clauses, Literal initialLiteral, bool succInSat) {
...
/*
void GlucoseParaProver::prepareLtlSat(LtlFormulaTriple clauses, Literal initialLiteral) {
  prepareLtlSat(clauses, Literal("$initial"), false);

  */

void GlucoseParaProver::prepareFalse() {
    calcSolver->addClause(~Glucose::mkLit(createOrGetVariable("$false")));
}

void GlucoseParaProver::prepareExtras(name_set extra) {
  for (string name : extra) {
    createOrGetVariable(name);//, Glucose::lbool((uint8_t)0));
  }
}

void GlucoseParaProver::prepareClauses(clause_set clauses) {
...
void GlucoseParaProver::prepareClauses(clause_list clauses) {
    // For the ltl prover
...
void GlucoseParaProver::prepareModalClauses(modal_clause_set modal_clauses,
                                        modal_names_map &newExtra,
...
void GlucoseParaProver::prepareLtlClauses(ltl_clause_list modal_clauses,
...
Glucose::Var GlucoseParaProver::createOrGetVariable(string name,
...
Glucose::Lit GlucoseParaProver::makeLiteral(shared_ptr<Formula> formula) {
...
Glucose::Lit GlucoseParaProver::makeLiteral(Literal literal) {
...
GlucoseParaProver::convertAssumptions(literal_set assumptions) {
...
bool GlucoseParaProver::modelSatisfiesAssump(Literal assumption) {
...
literal_set GlucoseParaProver::convertConflictToAssumps(
...
Solution GlucoseParaProver::solve(const literal_set &assumptions) {
  Solution solution;
...
Solution GlucoseParaProver::solveReduced(const literal_set &assumptions) {
    literal_set trigs;
...
void GlucoseParaProver::reduce_conflict(literal_set& conflict) {
...
void GlucoseParaProver::addClause(literal_set clause) {
  calcSolver->addClause(*convertAssumptions(clause));
}

void GlucoseParaProver::printModel() {
...
int GlucoseParaProver::getLiteralId(Literal literal) {
  return variableMap[literal.getName()];
}

literal_set GlucoseParaProver::getModel() {
    literal_set model;
    for (auto varName : nameMap) {
        model.insert(
